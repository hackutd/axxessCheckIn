...
export default function Reader({ callback, width, height }: QRCodeReaderProps) {
  ...
  const tick = () => {
    if (video.readyState === video.HAVE_ENOUGH_DATA) {
      setVideoReady(true);
      if (!canvas.current) return requestAnimationFrame(tick);
      ...
      if (!context) return requestAnimationFrame(tick);
  ...
  !paused &&
    navigator.mediaDevices
      .getUserMedia({
        video: { facingMode: 'environment', frameRate: 30, width, height },
      })
      .then((stream) => {
  ...
  return (
    <div className="flex items-center justify-center">
      {videoReady && !paused ? (
        ...
        <LoadIcon width={width} height={height} />
      )}
    </div>
  );
}
